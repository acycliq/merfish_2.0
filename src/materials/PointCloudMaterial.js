...
export class PointCloudMaterial extends THREE.RawShaderMaterial {
	...
	getDefines () {
		...
		if (this.shape === PointShape.SQUARE) {
			...
			defines.push('#define cross_point_shape');
		} else if (this.shape === PointShape.DIAMOND) {
			defines.push('#define diamond_point_shape');
		} else if (this.shape === PointShape.DOT) {
            defines.push('#define dot_point_shape');
        } else if (this.shape === PointShape.PLUS) {
            defines.push('#define plus_point_shape');
		} else if (this.shape === PointShape.MY_SQUARE) {
			defines.push('#define my_square_point_shape');
        } else if (this.shape === PointShape.STAR_5) {
            defines.push('#define star_5_point_shape');
        } else if (this.shape === PointShape.STAR_6) {
            defines.push('#define star_6_point_shape');
        } else if (this.shape === PointShape.TRIANGLE_DOWN) {
            defines.push('#define triangle_down_point_shape');
         }else if (this.shape === PointShape.TRIANGLE_UP) {
            defines.push('#define triangle_up_point_shape');
        } else if (this.shape === PointShape.TRIANGLE_LEFT) {
            defines.push('#define triangle_left_point_shape');
        } else if (this.shape === PointShape.TRIANGLE_RIGHT) {
            defines.push('#define triangle_right_point_shape');
        }
